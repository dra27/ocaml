...
LPLIST caml_win32unix_list_next (LPLIST lst)
{
  return lst->lpNext;
}

int caml_win32unix_list_length (LPLIST lst)
{
  int length = 0;
  LPLIST iter = lst;
  while (iter != NULL)
  {
    length++;
    iter = caml_win32unix_list_next(iter);
  };
  return length;
}

LPLIST caml_win32unix_list_concat (LPLIST lsta, LPLIST lstb)
{
  LPLIST res = NULL;
  LPLIST iter = NULL;
