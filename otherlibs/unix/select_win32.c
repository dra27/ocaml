                                     ...
                                     SELECTTYPE EType)
{
  ...
  /* Init common data */
  unix_list_init((LPLIST)res);
  unix_list_next_set((LPLIST)res, (LPLIST)lpSelectData);
  res->EType         = EType;
  res->nResultsCount = 0;
...
static void select_data_free (LPSELECTDATA lpSelectData)
{
  ...
  {
    unix_worker_job_finish(lpSelectData->lpWorker);
  ...
  */
  switch(get_handle_type(fd))
  {
    ...
    default:
      ...
      unix_win32_maperr(ERROR_INVALID_HANDLE);
                           ...
                           value timeout)
{
  ...
  if (readfds == Val_emptylist
    ...
    if (fdlist_to_fdset(readfds, &read)
      ...
      nEventsMax   = unix_list_length((LPLIST)lpSelectData);
      nEventsCount = 0;
          ...
          if (iterSelectData->funcWorker != NULL)
            {
              iterSelectData->lpWorker =
                unix_worker_job_submit(iterSelectData->funcWorker,
                                       (void *)iterSelectData);
              DEBUG_PRINT("Job submitted to worker %x",
                          iterSelectData->lpWorker);
              lpEventsDone[nEventsCount]
                = unix_worker_job_event_done(iterSelectData->lpWorker);
              nEventsCount++;
            };
          ...
          while (iterSelectData != NULL)
            {
              if (iterSelectData->lpWorker != NULL)
                {
                  unix_worker_job_stop(iterSelectData->lpWorker);
                };
              iterSelectData = LIST_NEXT(LPSELECTDATA, iterSelectData);
            };
