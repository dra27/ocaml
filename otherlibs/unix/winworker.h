 ...
 *   can be accessed through unix_worker_job_event_done.
 ...
 * submitted.
 */
LPWORKER unix_worker_job_submit (WORKERFUNC f, void *data);

/* Get event to know when a job is done.
 */
HANDLE unix_worker_job_event_done (LPWORKER);

/* Ask a job to stop processing.
 */
void unix_worker_job_stop (LPWORKER);

/* End a job submitted to worker.
 */
void unix_worker_job_finish (LPWORKER);
