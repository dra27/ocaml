...
static value convert_addrinfo(struct addrinfo * a)
{
  ...
  vaddr = unix_alloc_sockaddr(&sa, len, -1);
  vcanonname = caml_copy_string(a->ai_canonname == NULL ? "" : a->ai_canonname);
  vres = caml_alloc_small(5, 0);
  Field(vres, 0) =
    unix_cst_to_constr(a->ai_family, unix_socket_domain_table, 3, 0);
  Field(vres, 1) =
    unix_cst_to_constr(a->ai_socktype, unix_socket_type_table, 4, 0);
  ...
  for (/*nothing*/; Is_block(vopts); vopts = Field(vopts, 1)) {
    ...
    if (Is_block(v))
      ...
      case 0:                   /* AI_FAMILY of socket_domain */
        hints.ai_family = unix_socket_domain_table[Int_val(Field(v, 0))];
        break;
      case 1:                   /* AI_SOCKTYPE of socket_type */
        hints.ai_socktype = unix_socket_type_table[Int_val(Field(v, 0))];
        break;
