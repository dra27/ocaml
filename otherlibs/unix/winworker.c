...
DWORD WINAPI unix_worker_wait (LPVOID _data)
{
  BOOL     bExit;
  LPWORKER lpWorker;
...
void unix_worker_free (LPWORKER lpWorker)
{
...
void unix_worker_push(LPWORKER lpWorker)
{
  BOOL bFreeWorker;
  ...
  if (unix_list_length((LPLIST)lpWorkers) < THREAD_WORKERS_MAX)
  {
    DEBUG_PRINT("Saving this worker for future use");
    DEBUG_PRINT("Next: %x", ((LPLIST)lpWorker)->lpNext);
    lpWorkers = (LPWORKER)unix_list_concat((LPLIST)lpWorker, (LPLIST)lpWorkers);
    bFreeWorker = FALSE;
  };
  nWorkersCurrent--;
  DEBUG_PRINT("Workers running current/running max/waiting: %d/%d/%d",
  ...
  if (bFreeWorker)
  {
    ...
    unix_worker_free(lpWorker);
  }
}

void unix_worker_init (void)
{
  int i = 0;
...
LPWORKER unix_worker_job_submit (WORKERFUNC f, void *user_data)
{
  LPWORKER lpWorker = unix_worker_pop();
...
HANDLE unix_worker_job_event_done (LPWORKER lpWorker)
{
  return lpWorker->hJobDone;
}

void unix_worker_job_stop (LPWORKER lpWorker)
{
...
void unix_worker_job_finish (LPWORKER lpWorker)
{
