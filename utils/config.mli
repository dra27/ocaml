...
(** The vendor and version of the C compiler. Consists of hyphenated values
    ...
    Possible values are:
    - {v msvc v} - Windows, using Microsoft Visual Studio. {v _MSC_VER v} and
                   optionally {v clang- v} if {v clang-cl v}, rather than
                   Visual Studio itself, is in use.
    - {v icc v} - Intel C Compiler.
    - {v mingw v} - Windows, using the mingw-w64 project. The major and minor
                    version of mingw-w64 itself follows and then either
...
val suffixing : bool
(** Whether the runtime executable filenames are being mangled with Runtime IDs
    and the {!target}.
