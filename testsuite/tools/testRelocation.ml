      ...
      if String.ends_with ~suffix:".opt" basename
        ...
        let linked_with_debug =
          (* All bytecode executables are linked with -g, except extract_crc
             ...
             (until #11147 in 5.0). *)
          let bytecode_executables =
            ["extract_crc"; "flexlink.byte"; "ocamllex.byte"; "ocamldoc"] in
          let bytecode_executables =
            if config.has_ocamlopt then
              bytecode_executables
            else
              "flexlink" :: "ocamllex" :: bytecode_executables
          in
          not (List.mem basename bytecode_executables)
        in
  ...
  if basename = "expunge" || basename = "expunge.exe"
    ...
    let (embeds_stdlib_location, has_ocaml_debug_info, has_c_debug_info,
      ...
      if basename = "Makefile.config" || basename = "ld.conf" then
        ...
        let stdlib_from_flexdll = (* via FlexDLL bootstrap *)
          (* Prior to #13070 in 5.3.0, the compiler still added +flexdll to the
             search path even when -nostdlib was specified. That means that
             when FlexDLL is bootstrapped, everything compiled with the root
             ocamlc or with ocamlopt will contain the standard library
             directory. It's slightly complex for the profiling object and
             compiler-libs - the .cmti files are always compiled with
             boot/ocamlc, but if the native compiler is available, then most
             modules are recompiled using ocamlopt, and this will cause the .cmt
             from boot/ocamlc to be replaced. Although it is nearly the case
             that a .cmt with no .cmx will have been compiled by ocamlc-only,
             the backend-agnostic toplevel modules use the .cmt from the
             bytecode version and the .cmx files for the native versions aren't
             installed! *)
          if Config.flexdll_dirs <> [] then
            if Filename.basename (Filename.dirname file) = "compiler-libs"
               || Filename.chop_extension basename = "profiling" then
              let ocamlc_only =
                ["config_boot.cmt";
                 "config_main.cmt";
                 "expunge.cmt";
              ...
              ext = ".cmt"
              && config.has_ocamlopt
              && not (List.mem basename ocamlc_only)
            else
              Filename.chop_extension basename <> "topdirs"
              && (ext = ".cmt" || ext = ".cmti")
          else
            false in
        let stdlib_from_config = (* via Config.standard_library *)
          List.mem basename ["config.cmt"; "dynlink.cma"; "ocamlcommon.cma"] in
        let stdlib = stdlib_from_flexdll || stdlib_from_config in
        ...
        (* Prior to #8537 in 4.09.0, the additional headers were installed with
           the wrong name *)
      else if String.starts_with ~prefix:"target_camlheader" basename
              || basename = "camlheader" then
        let stdlib = config.shebangscripts && basename <> "camlheader_ur" in
    ...
    let contains_build_path =
      ...
      if String.starts_with ~prefix:"libasmrun" basename then
        ...
        (* The .cmxs files are all built without -g prior to #9804 in
           OCaml 4.12.0 *)
        contains_assembled_objects
        && not Harness.is_macos && Config.ccomp_type <> "msvc"
        && Toolchain.assembler_embeds_build_path
