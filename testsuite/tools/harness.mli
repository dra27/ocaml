  ...
  type config = {
    ...
    launcher_searches_for_ocamlrun: bool;
      (** Indicates whether bytecode executables in the compiler distribution
          ...
          used to be the behaviour for native Windows. *)
...
(** [true] if the [caml_executable_name] runtime function is implemented for
    this platform and returns an absolute path to the currently running
    executable. *)
...
(** [files_for ~source_and_cmi mode name files] adds filenames to files for the
    {i name}{v .ml v} and {i name}{v .cmi v} if [~source_and_cmi:true] and also
    {i name}{v .cmo v} or {i name}{v .cmx v} depending on [mode]. If [mode] is
    [Native], then the assembled object is also added. *)
