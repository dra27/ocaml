  ...
  let make ?(caml_ld_library_path=false) ?(ocamllib=false) bindir libdir =
    ...
    let bindings =
      ...
      || List.exists (String.starts_with ~prefix:"LD_LIBRARY_PATH=")
        ...
        ("LD_LIBRARY_PATH=" ^ libdir)::bindings
    in
    let bindings =
      if ocamllib then
  ...
  let rec run_process_aux ~to_stdout ?runtime program args ?(no_stderr = false)
    ...
    let runtime, classification =
      ...
      | None ->
          "", `Not_tendered_bytecode
      | Some runtime ->
          runtime, classify_bytecode_image program
    in
    let summarise f () =
  ...
  if config.has_ocamlopt then
    ...
    In_channel.with_open_bin lib (fun ic ->
      try
        ...
        let libs =
          (input_value ic : Cmx_format.library_infos).Cmx_format.lib_ccobjs
        in
        if libs <> [] then
          String.concat " " (List.rev libs)
        else
          ""
      with End_of_file ->
  ...
  let test_binary binary =
    ...
    if is_executable binary then
      ...
      | `Shebang | `Launcher | `Custom ->
          Environment.run_process Stdout ?runtime binary ["-vnum"] env
  in
...
let compile_test ~original env bindir =
  ...
  fun test test_program description ->
    let needs_shared_support, needs_ocamlopt, options, scheme, clibs =
      match test with
      | Default C_ocamlc ->
      ...
      | Custom Static ->
          false, false, ["-custom"], None, None
      | Custom Shared ->
          true, false, ["-custom"; "-runtime-variant"; "_shared"], None, None
      | Output_obj(C_ocamlc, Static) ->
          false, false, ["-output-obj"], Some "", None
      | Output_obj(C_ocamlc, Shared) ->
          false, false, ["-output-obj"], Some "_shared", None
      | Output_obj(C_ocamlopt, Static) ->
          false, true, ["-output-obj"],
          Some "", Some ocamlcommon_native_c_libraries
      | Output_obj(C_ocamlopt, Shared) ->
          true, true, ["-output-obj"],
          Some "_shared", Some ocamlcommon_native_c_libraries
      | Output_complete_obj(C_ocamlc, Static) ->
          false, false, ["-output-complete-obj"], Some "", None
      ...
      | Output_complete_exe Shared ->
          true, false, ["-output-complete-exe"; "-runtime-variant"; "_shared"],
          None, None
    in
    (* At present, shared runtime support is not available on Windows *)
    if needs_shared_support
    ...
    || needs_ocamlopt && not config.has_ocamlopt then
      ...
      let compile_with_main_in_c output suffix =
        let runtime_lib =
      ...
      let () =
        ...
        in
        List.iter Sys.remove files;
        Option.iter (compile_with_main_in_c output) scheme
      in
