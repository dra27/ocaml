...
(* XXX TODO Relocatable test: Sys.getcwd returns the Physical working dir, but
            we should also check for the Logical working dir (OCaml embeds the
            physical; GCC embeds the logical)
       Very visible on FreeBSD where /home -> /usr/home
       NB Windows and Unix are the opposite way around; on Unix, $PWD will be
          the logical CWD and Sys.getcwd () will return the physical CWD
          On Windows, Sys.getcwd () returns the logical CWD and this can then
          be passed to Unix.realpath to get the physical CWD *)
...
(* Environment augmentations which can be applied using Environment.make *)
type env_augmentation =
| Shim of {caml_ld_library_path: string option;
           ocamllib: string option}
| Test of {caml_ld_library_path: string list option;
           ocamllib: string option;
...
module Environment : sig
  ...
  val make : ?env:env_augmentation -> string -> string -> t
  ...
  let make ?env bindir libdir =
    ...
    in
    let caml_ld_library_path, ocamllib, camllib, testing =
      match env with
      | None -> None, None, None, false
      | Some (Test {caml_ld_library_path; ocamllib; camllib}) ->
          caml_ld_library_path, ocamllib, camllib, true
      | Some (Shim {caml_ld_library_path; ocamllib}) ->
          Option.map (fun x -> [x]) caml_ld_library_path, ocamllib, None, false
    in
    ...
    with Not_found ->
      ...
      {env; serial; bindir; libdir; augmentations; testing}
...
@{<inline_code>> @}  LD_LIBRARY_PATH=%a:$LD_LIBRARY_PATH\n"
    ...
    try
      ...
      | Unix.WSIGNALED n when not to_stdout && n = Sys.sigabrt ->
...
|} libdir
  ...
  let expected_exit_code =
    (* Tendered Relocatable OCaml bytecode executables require
       caml_executable_name, or they end up being accidentally relative (since
       ocamlrun can't determine where it is). After the prefix has been renamed,
       CAML_LD_LIBRARY_PATH has to be set, since ld.conf is incorrect, so this
       issue doesn't appear (i.e. it doesn't matter that ocamlrun can't find
       ld.conf because its content is useless anyway) *)
    if mode = Bytecode && config.has_relative_libdir <> None
       && no_caml_executable_name && original
...
let () =
  ...
  let print s = if s = "" then print_endline "." else print_endline s in
      ...
      if code = 0 then
        let lines =
          ...
          if Sys.win32 then
            if (test.camllib = Empty
                  && (original || config.has_relative_libdir <> None)
        ...
        let lines =
          (* Known issue: Dll.ld_conf_contents preserves NUL characters in lines
             where caml_parse_ld_conf terminates processing *)
          let includes_nulls =
...
let test_ld_conf ~original env bindir libdir =
  ...
  let run_test test =
    ...
    let env = Test {caml_ld_library_path; ocamllib; camllib} in
    let env = Environment.make ~env bindir libdir in
    ...
    || needs_ocamlopt && not config.has_ocamlopt then
      ...
      let options =
        if original && config.has_relative_libdir = None then
          ...
          match test with
          | Default_ocamlc ->
              (* XXX Temp - fairly sure "Temp" here refers to matching on test,
                            not the fact we test -dllpath! *)
                ...
                let outcome =
                  ...
                  if runtime <> None then
                    ...
                    | Tendered ->
                        ...
                        else if no_caml_executable_name && original
                                && config.has_relative_libdir <> None then
...
let test_relocation env prefix bindir libdir =
  ...
  Option.iter (Printf.printf "  Relative Prefix: %s\n") relative_libdir;
  ...
  let in_unexpected_state file file_rel rules =
    ...
    let gather acc (d, _) =
      if List.mem d seen then
        match d with
        | Build_dir(kind, enc) ->
            LocationSet.add Build acc,
            some_string "Build directory (%a)" string_of_build_dir (kind, enc)
        | Prefix_dir enc ->
            LocationSet.add Prefix acc,
            some_string "Installation prefix (%a)" string_of_encoding enc
        | Relative_libdir enc ->
            LocationSet.add Relative acc,
            some_string "Relative prefix (%a)" string_of_encoding enc
      else
        acc, None in
      ...
      if config.has_relative_libdir <> None then
        ...
        let files =
          if Config.flambda then
            ...
            ("compiler-libs" / "config.cmx") :: files
        in
        "expunge" ::
        "runtime-launch-info" ::
  ...
  let libdir_files_with_relative_prefix =
    if config.has_relative_libdir = None then
      ...
      let files = ["ld.conf"; "Makefile.config"] in
      StringSet.of_list (List.map (Filename.concat libdir) files) in
...
let () =
  ...
  let env =
    let env = Shim {caml_ld_library_path; ocamllib = None} in
