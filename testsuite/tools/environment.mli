...
(** Test Environments. A test environment is a installation of OCaml (in a given
    prefix) compiled in a build path packaged with a process environment in
...
(** Constructs an environment with the given formatter for displaying paths and
    ...
    The result can be used {!run_process} and {!run_process_with_test_env} and
    scrubs the process environment, ensuring that bindir is present in
