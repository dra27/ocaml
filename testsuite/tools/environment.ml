  ...
  try
    In_channel.with_open_bin file (fun ic ->
      ...
      if start = "#!" then
        ...
        let rntm =
          try Bytesections.read_section_string ic "RNTM"
        ...
        let dlls = Bytesections.read_section_string ic "DLLS" in
...
let input_artefact_from_file env file =
  In_channel.with_open_bin file @@ fun ic ->
    ...
    | ext when (ext = Config.ext_lib || ext = Config.ext_obj)
               && Sys.os_type = "Unix" && not Harness.is_macos ->
        let exit, lines =
