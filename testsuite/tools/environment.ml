...
let classify_executable file =
  try
    In_channel.with_open_bin file (fun ic ->
      ...
      | Bytesections.{name = Name.DLLS} -> true
      | _ -> false
      in
      ...
      if start = "#!" then
        ...
        Tendered {header = Header_shebang;
                  ...
                  runtime}
      else if List.exists is_DLLS sections then
        let rntm =
          try Bytesections.read_section_string toc ic Bytesections.Name.RNTM
          with Not_found -> "ocamlrun\000"
        in
        ...
        let dlls =
          Bytesections.read_section_string toc ic Bytesections.Name.DLLS
        in
...
let run_one (runtime, quiet, fails, program, argv0, args,
  ...
  let lines =
    ...
    (* Some of the tests send lines of text which end with '\r'. On native
       ...
       and text mode will _correctly_ translate that to "\r\n" (and the caller
       receives a line ending with '\r').
