...
let run_process ?(runtime = false) ?(stubs = false) ?(stdlib = false)
  ...
  (* Calculate the strategy. This is a non-empty list of environments to be
     tried. Each strategem controls whether the program should be executed via
     ocamlrun and if an augmented environment should be used. *)
  let strategy =
