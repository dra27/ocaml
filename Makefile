...
flexlink.byte$(EXE): $(FLEXDLL_SOURCE_FILES) | boot/bin.byte
	...
	rm -f boot/bin.byte/flexlink$(EXE)
	cat boot/ocamlruns$(EXE) flexlink.byte$(EXE) > boot/bin.byte/flexlink$(EXE)
	cp $(addprefix $(FLEXDLL_SOURCES)/, $(FLEXDLL_OBJECTS)) boot/bin.byte/
...
flexlink.opt$(EXE): $(FLEXDLL_SOURCE_FILES) stdlib/stdlib.cmxa flexlink.byte$(EXE) | boot/bin.opt
	...
	rm -f boot/bin.opt/flexlink$(EXE)
	cd boot/bin.opt; $(LN) ../../$@ flexlink$(EXE)
	cp $(addprefix $(FLEXDLL_SOURCES)/, $(FLEXDLL_OBJECTS)) boot/bin.opt/
