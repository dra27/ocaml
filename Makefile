...
OCAML_BYTECODE_PROGRAMS = expunge \
  $(TOOLS_BYT_PROGRAMS) $(addprefix tools/, cvt_emit ocamltex)
...
FLEXLINK_BUILD_ENV = \
  ...
  CHAINS=$(FLEXDLL_CHAIN) ROOTDIR=.. DIR_SEP=$(DIR_SEP) SHELL=$(SHELL)
...
boot/bin.byte boot/bin.opt:
	$(call MKDIR,$@)

flexlink.byte$(EXE): $(FLEXDLL_SOURCE_FILES) | boot/bin.byte
	$(call RM_F, $(FLEXDLL_SOURCES)/flexlink.exe)
	$(MAKE) -C $(FLEXDLL_SOURCES) $(FLEXLINK_BUILD_ENV) \
	  OCAMLRUN=$(call SHELL_QUOT,$$(ROOTDIR)$(DIR_SEP)boot$(DIR_SEP)ocamlruns$(EXE)) NATDYNLINK=false \
	  OCAMLOPT=$(call SHELL_QUOT,$(value BOOT_OCAMLC) $(USE_RUNTIME_PRIMS) $(USE_STDLIB)) \
	  flexlink.exe support
	$(call CP, $(FLEXDLL_SOURCES)/flexlink.exe, flexlink.byte$(EXE))
	$(call RM_F, boot/bin.byte/flexlink$(EXE))
	$(call CAT_TO, boot/ocamlruns$(EXE) flexlink.byte$(EXE), boot/bin.byte/flexlink$(EXE))
	$(call CP, $(addprefix $(FLEXDLL_SOURCES)/, $(FLEXDLL_OBJECTS)), boot/bin.byte/)

boot/bin.byte/flexlink$(EXE): | flexlink.byte$(EXE)
...
runtime/ocamlrun$(EXE): | boot/bin.byte/flexlink$(EXE)
endif

# Start up the system from the distribution compiler
.PHONY: coldstart
coldstart: $(COLDSTART_STDLIB_RUNTIME)
	$(MAKE) -C stdlib OCAMLRUN=$(call SHELL_QUOT,$$(ROOTDIR)/$<) \
	  CAMLC=$(call SHELL_QUOT,$$(BOOT_OCAMLC) $(USE_RUNTIME_PRIMS)) all
	$(MAKE) setup-boot

.PHONY: setup-boot
setup-boot: runtime/ocamlrun$(EXE)
	$(call RM_F, $(addprefix boot/, ocamlrun$(EXE) libcamlrun.$(A) $(LIBFILES)))
	$(call CP, $<, boot/ocamlrun$(EXE))
	$(call CP, $(addprefix stdlib/, $(LIBFILES)), boot)
...
flexlink.opt$(EXE): $(FLEXDLL_SOURCE_FILES) stdlib/stdlib.cmxa flexlink.byte$(EXE) | boot/bin.opt
	$(call RM_F, $(FLEXDLL_SOURCES)/flexlink.exe)
	$(MAKE) -C $(FLEXDLL_SOURCES) $(FLEXLINK_BUILD_ENV) \
	  OCAMLOPT=$(call SHELL_QUOT,$(subst /,$(DIR_SEP),$(FLEXLINK_OCAMLOPT)) -nostdlib -I ../stdlib) flexlink.exe
	$(call CP, $(FLEXDLL_SOURCES)/flexlink.exe, $@)
	$(call RM_F, boot/bin.opt/flexlink$(EXE))
	$(call LN_IN, boot/bin.opt, ../../$@, flexlink$(EXE))
	$(call CP, $(addprefix $(FLEXDLL_SOURCES)/, $(FLEXDLL_OBJECTS)), boot/bin.opt/)
...
ocaml.tmp: $(ocaml_LIBRARIES:=.cma) $(ocaml_MODULES:=.cmo) | runtime/primitives
...
lambda/runtimedef.ml: lambda/runtimedef.ml.c runtime/caml/builtin_exns.h \
...
runtime_BUILT_HEADERS = runtime/build_config.h runtime/primitives.h
...
SOURCES_WITH_PRIMITIVES = \
  alloc.c array.c compare.c extern.c floats.c gc_ctrl.c hash.c intern.c ints.c \
  io.c lexing.c md5.c meta.c memprof.c obj.c parsing.c signals.c str.c sys.c \
  callback.c weak.c finalise.c domain.c fiber.c memory.c runtime_events.c \
  sync.c dynlink.c backtrace_byt.c backtrace.c afl.c bigarray.c prng.c
...
# of building runtime/ocamlruns. The order-only dependency here means that when
# runtime/ocamlrun is then compiled, we don't invalidate runtime/primitives. In
...
runtime/ocamlruns$(EXE): runtime/prims.$(O) runtime/libcamlrun_non_shared.$(A)
	...
	$(V_GEN)runtime$(DIR_SEP)ocamlruns$(EXE) -p > runtime/primitives
...
clean::
	...
	rm -f runtime/primitives runtime/primitives.new $(runtime_BUILT_HEADERS)
...
ifeq "$(INSTALL_BYTECODE_PROGRAMS)" "true"
	...
	$(INSTALL_DATA) $(addprefix boot/bin.byte/, $(FLEXDLL_OBJECTS)) \
    "$(INSTALL_FLEXDLLDIR)"
