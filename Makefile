...
install::
	...
	$(call INSTALL_MKDIR, lib, $(INSTALL_LIBDIR_PROFILING))
...
ifeq "$(INSTALL_BYTECODE_PROGRAMS)" "true"
	if test -f "tools/$(1)".opt$(EXE); then \
	  $$(call INSTALL_ITEM, \
	    tools/$(1)$(EXE), bin, , $(1).byte$(EXE)); \
	  $$(call INSTALL_ITEM, \
	    tools/$(1).opt$(EXE), bin, , , $(1)$(EXE)); \
	else \
	  $(call INSTALL_ITEM, \
	    tools/$(1)$(EXE), bin, , $(1).byte$(EXE), $(1)$(EXE)); \
	fi
else
	if test -f "tools/$(1)".opt$(EXE); then \
	  $$(call INSTALL_ITEM, \
...
# dynlink is removed from the previous installation.
	...
	$(call INSTALL_MKDIR, lib, $(INSTALL_LIBDIR_DYNLINK))
	$(call INSTALL_ITEMS, \
...
ifeq "$(build_ocamldoc)" "true"
	$(call INSTALL_MKDIR, lib, $(INSTALL_LIBDIR_OCAMLDOC))
...
ifeq "$(INSTALL_BYTECODE_PROGRAMS)" "true"
	if test -f flexlink.opt$(EXE); then \
	  ...
	  $(call INSTALL_ITEM, \
	    flexlink.byte$(EXE), bin, , , flexlink$(EXE)); \
	fi
...
ifeq "$(INSTALL_BYTECODE_PROGRAMS)" "true"
	$(call INSTALL_STRIPPED_BYTE_PROG, ocamlopt$(EXE), bin, ocamlopt.byte$(EXE))
endif
