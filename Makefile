...
ifneq "$(wildcard ocamltest)" ""
DEP_DIRS += ocamltest testsuite/lib testsuite/tools
endif
...
$(filter-out $(wildcard $(OPTIONAL_DIRECTORIES)), $(OPTIONAL_DIRECTORIES)):
	...
	@echo "This happens when the sources of OCaml are extracted from a tarball"
	@echo "generated by git-archive (which includes those generated by GitHub)"
	@head -n 1 VERSION | grep -Fq + || \
   echo "Note that the release tarballs published at \
https://caml.inria.fr/pub/distrib/ include all the manual and testsuite sources"
	@$(if $(GIT_ARCHIVE_SHA),,false)
	@echo "The required files are in commit $(GIT_ARCHIVE_SHA), for example:"
	@echo "  git clone https://github.com/ocaml/ocaml \
--revision $(GIT_ARCHIVE_SHA) --depth 1 git-sources"
	@echo "  mv git-sources/$@ ."
	@false
