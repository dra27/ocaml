...
struct caml_params {
  ...
  /* The full filename to use as the basis for determining the Standard Library
     ...
     systems where caml_executable_name returns a non-NULL value, this is the
     same string as proc_self_exe, otherwise it will be the _actual_ argv[0]
     passed to the process (which for bytecode may mean the interpreter). */
