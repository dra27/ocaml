...
CAMLexport int caml_multi_byte_to_wide_char(const char *s, int slen,
...
CAMLexport int caml_wide_char_to_multi_byte(const wchar_t *s, int slen,
                                            char *out, int outlen)
{
  int retcode;
...
CAMLexport value caml_copy_string_of_utf16(const wchar_t *s)
{
  ...
  retcode = caml_wide_char_to_multi_byte(s, slen, NULL, 0);
  v = caml_alloc_string(retcode);
  caml_wide_char_to_multi_byte(s, slen, (char *)String_val(v), retcode);
...
CAMLexport wchar_t* caml_stat_strdup_to_utf16(const char *s)
{
  ...
  retcode = caml_multi_byte_to_wide_char(s, -1, NULL, 0);
  ws = caml_stat_alloc_noexc(retcode * sizeof(*ws));
  caml_multi_byte_to_wide_char(s, -1, ws, retcode);
...
CAMLexport caml_stat_string caml_stat_strdup_of_utf16(const wchar_t *s)
{
  ...
  retcode = caml_wide_char_to_multi_byte(s, -1, NULL, 0);
  out = caml_stat_alloc(retcode);
  caml_wide_char_to_multi_byte(s, -1, out, retcode);
