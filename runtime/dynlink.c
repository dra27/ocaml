...
static char_os *expand_entry(char_os *libroot, char_os *dir)
{
  if (dir[0] == '.') {
    if (dir[1] == '\0') {
      /* dir is exactly "." => return libroot */
      return caml_stat_strdup_os(libroot);
    } else if (Is_separator(dir[1])) {
      if (dir[2] == '\0') {
        /* dir is exactly "./" => return libroot */
        return caml_stat_strdup_os(libroot);
      } else {
        /* dir begins "./" => replace the ./ with libroot (which has a trailing
           slash) */
        return caml_stat_strconcat_os(2, libroot, (dir + 2));
      }
    } else if (dir[1] == '.' && (dir[2] == '\0' || Is_separator(dir[2]))) {
      /* dir is either exactly ".." or begins "../" => prefix it with libroot
         (which has a trailing slash) */
      return caml_stat_strconcat_os(2, libroot, dir);
    } else {
      /* dir is not relative, but simply begins with a dot => return a copy */
      return caml_stat_strdup_os(dir);
    }
  } else {
    /* dir is not relative => return a copy */
    return caml_stat_strdup_os(dir);
  }
}
  ...
  for (int i = 0; i < sizeof(locations) / sizeof(locations[0]); i++) {
    if (locations[i] != NULL) {
      ...
      while (*p != '\0') {
        ...
        *q = '\0';
        char_os *entry = expand_entry(libroot, p);
