...
rem appveyor DownloadFile "https://github.com/ocaml/flexdll/archive/%FLEXDLL_VERSION%.tar.gz" -FileName "flexdll.tar.gz" || exit /b 1
rem appveyor DownloadFile "https://github.com/ocaml/flexdll/releases/download/%FLEXDLL_VERSION%/flexdll-bin-%FLEXDLL_VERSION%.zip" -FileName "flexdll.zip" || exit /b 1
...
rem mkdir "%GITHUB_WORKSPACE%\..\flexdll"
rem move flexdll.zip "%GITHUB_WORKSPACE%\..\flexdll"
rem cd "%GITHUB_WORKSPACE%\..\flexdll" && unzip -q flexdll.zip
...
"%CYG_ROOT%\bin\bash.exe" -lc "$GITHUB_WORKSPACE/tools/ci/appveyor/appveyor_build.sh install" || exit /b 1

goto :EOF

:build
"%CYG_ROOT%\bin\bash.exe" -lc "$GITHUB_WORKSPACE/tools/ci/appveyor/gha_build.sh" || exit /b 1
goto :EOF
...
call :SetDefaultSDK
"%CYG_ROOT%\bin\bash.exe" -lc "$GITHUB_WORKSPACE/tools/ci/appveyor/appveyor_build.sh test" || exit /b 1
goto :EOF
