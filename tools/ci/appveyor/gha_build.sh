...
GITHUB_WORKSPACE=$(echo "$GITHUB_WORKSPACE" | cygpath -f -)
...
case "$1" in
  install)
    ...
    ;;
  test)
    FULL_BUILD_PREFIX="$GITHUB_WORKSPACE/../$BUILD_PREFIX"
    ...
    ;;
  *)
    cd "$GITHUB_WORKSPACE/../$BUILD_PREFIX-$PORT"
    ...
    if [[ $install_flexdll = 'true' ]] ; then
      tar -xzf "$GITHUB_WORKSPACE/flexdll.tar.gz"
