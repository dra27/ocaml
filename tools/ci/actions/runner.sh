...
Build () {
  ...
  if [ "$(uname)" = 'Darwin' ]; then
    script -q build.log $MAKE_WARN world.opt || failed=$?
    if ((failed)); then
      script -q build.log $MAKE_WARN -j1 V=1 world.opt
      exit $failed
    fi
    script -qa build.log $MAKE_WARN ocamlnat
  else
    script --return --command "$MAKE_WARN world.opt" build.log || failed=$?
    if ((failed)); then
      script --return --command "$MAKE_WARN -j1 V=1 world.opt" build.log
      exit $failed
    fi
