...
if [ x"$make_args" = 'xinstall' ]; then
  ...
  if [ -e 'config.status' ]; then
    ...
    origin_prefix="$(opam var --switch="$origin" prefix | tr -d '\r')"
    echo "Cloning from $origin_prefix"
...
if [ x"$cloning" = 'xenabled' ]; then
  echo "Searching for a switch containing build-id $build_id"

  if [ -e "$OPAMSWITCH/share/ocaml/build-id" ]; then
    switch="$(tail -n 1 "$OPAMSWITCH/share/ocaml/build-id")"
    if [ -n "$switch" ]; then
      switch_share_dir="$(opam var --switch="$switch" share | tr -d '\r')"
  ...
  opam switch list --short | tr -d '\r' | grep -Fxv "$OPAMSWITCH" \
    >> opam-switches 2> /dev/null
    ...
    if [ -e "$switch_build_id" ]; then
      if [ x"$build_id" = x"$(head -n 1 "$switch_build_id")" ]; then
        ...
        if cp --reflink=always "$switch_build_id" __cp_test 2>/dev/null; then
          rm __cp_test
          echo "$origin: copy-on-write supported"
          origin="$switch"
          ...
          if [ -z "$clone_mechanism" ]; then
            echo "$origin: hard linking supported"
            origin="$switch"
            clone_mechanism='hard-linking'
          fi
        elif [ -z "$origin" ]; then
          echo "$origin: copying supported"
          origin="$switch"
        fi
        origin="$switch"
      fi
...
if [ -n "$origin" ]; then
  ...
  echo "Clone compiler from: $origin"
...
EOF
  ...
  echo "No suitable compiler found - building from sources"
