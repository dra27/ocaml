...
let process_clone oc process =
  ...
  in
  Array.iter process_file (Sys.readdir Filename.current_dir_name)
  ...
  if Sys.file_exists file then
    ...
    if not Sys.win32 then
      let ln (dir, target, source) =
        let target = Filename.quote target in
        let source = Filename.quote (Filename.concat dir source) in
        output_endline oc {|%s %s %s|} ln_command target source
      in
      List.iter ln lines
    else begin
      ...
      and cp (dir, target, source) =
        let target = Filename.quote (Filename.concat dir target) in
        let source = Filename.quote (Filename.concat dir source) in
        output_endline oc {|  $CP %s %s|} target source
      in
...
set -e|};
    ...
    (* Don't pass -p to cp on Windows - it's never going to be relevant (no
       ...
       more likely to fail than add anytning useful (especially if copying from
...
if cp --reflink=always share/doc/ocaml/LICENSE "$1/__cp_test" 2>/dev/null; then
  rm -f "$1/__cp_test"
  ...
  CP='cp -%sf'
  if ! test -e "$1/clone-files"; then
    if ln -f share/doc/ocaml/LICENSE "$1/__ln_test" 2>/dev/null; then
      rm -f "$1/__ln_test"
      ...
      let has_config_cache = Sys.file_exists "config.cache" in
      if has_config_cache then begin
        Out_channel.with_open_text "clone-share@ocaml" @@ fun oc ->
          output_string oc "share/ocaml/config.cache"
      end;
      process_clone oc clone_files;
      if not has_config_cache then
        output_endline oc {|mkdir -p "$1/share/ocaml"|};
