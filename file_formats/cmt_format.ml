  ...
  if !Clflags.binary_annotations && not !Clflags.print_types then begin
    Misc.output_to_file_via_temporary
       ...
       (fun temp_file_name oc ->
         ...
         let cmt_args =
           let base_argv0 =
             (* Avoid embedding any absolute path in Sys.argv.(0) *)
             let base_argv0 = Filename.basename Sys.argv.(0) in
             if Sys.win32 && String.ends_with ~suffix:".exe" base_argv0 then
               Filename.chop_extension base_argv0
             else
               base_argv0 in
           let cmt_args = Array.copy Sys.argv in
           cmt_args.(0) <- base_argv0;
           cmt_args in
         let cmt = {
