...
module Make (Arch : Operations.S) (Proc : module type of Processor) = struct
  ...
  module Coeffect = Coeffect
  module Effect_and_coeffect = Effect_and_coeffect
  ...
  *)
  ...
  method emit_expr (env:environment) exp =
    ...
    | Cvar v ->
        ...
        with Not_found ->
          Misc.fatal_errorf
            "Selection.emit_expr: unbound var %s" (V.unique_name v)
        end
    | Clet(v, e1, e2) ->
      ...
      | Effect.None ->
        ...
        | Coeffect.Arbitrary -> begin
          (* Arbitrary expressions may only be deferred if evaluation of
             every [exp'] (for [exp'] as in the comment above) has no
             effects. *)
          match EC.effect effects_after with
          | Effect.None -> true
  ...
  method emit_fundecl ~future_funcnames f =
    ...
    instr_iter
      (fun instr -> self#mark_instr instr.Mach.desc) body_with_prologue;
