  ...
  let size_expr (env:environment) exp =
    let rec size localenv = function
      ...
      | Cvar id ->
          ...
          with Not_found ->
            Misc.fatal_errorf "Selection.size_expr: unbound var %s"
                              (V.unique_name id)
          end
      | Ctuple el ->
  ...
  method emit_expr (env:environment) exp =
    ...
    | Cvar v ->
        begin try
          Some(env_find v env)
        with Not_found ->
          Misc.fatal_errorf "Selection.emit_expr: unbound var %s"
                            (V.unique_name v)
        end
    | Clet(v, e1, e2) ->
    ...
    | Cexit (nfail,args) ->
        ...
        | Some (simple_list, ext_env) ->
            ...
            let dest_args =
              ...
              with Not_found ->
                Misc.fatal_errorf "Selection.emit_expr: unbound label %d" nfail
            in
