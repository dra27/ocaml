...
let emit_frames a =
  ...
  let fully_pack_info fd_raise d has_next =
    (* Fully packed debuginfo:
       0lllllmaaabbboooooddddddddddddkn
         0           - d points to a name_info struct
         l (12 bits) - start line number
         m (3 bits)  - offset of end line number from start
         a (6 bits)  - start character offset relative to start line
         b (7 bits)  - end character offset relative to end line
         o (9 bits)  - difference between start bol and end bol
                       (so o + b is the end character relative to start line)
         d (24 bits) - memory offset to name_info struct
