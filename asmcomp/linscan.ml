...
let allocate_registers (intervals : Interval.result) =
  ...
  let slot_of_spilled i =
    ...
    | Stack(Local ss) -> ss
    | _ -> invalid_arg "Linscan.slot_of_spilled" in
  ...
  let split_by_pos intervals pos =
    let divider =
      (* this interval is strictly above intervals [i] with [i.iend < pos] and
         ...
         intervals in the set (according to the equality function of
         [IntervalSet] above). *)
  ...
  let partition_live intervals pos =
    IntervalSet.partition (fun i -> Interval.is_live i pos) intervals in
  ...
  let release_expired_active ci pos =
    ...
    ci.ci_active <- active;
    ci.ci_inactive <- IntervalSet.union inactive ci.ci_inactive in
  ...
  let release_expired_inactive ci pos =
    ...
    ci.ci_active <- IntervalSet.union active ci.ci_active in
  ...
  let allocate_stack_slot num_stack_slots i =
    ...
    i.reg.spill <- true;
    ci.ci_spilled <- IntervalSet.add i ci.ci_spilled in
        ...
        | rn ->
            ...
            (* Create register mask for this class
               note: if frame pointers are enabled then some registers may have
                     indexes that are off-bounds; we hence protect write
                     accesses below (given that the assign function will not
                     consider such registers) *)
  ...
  let allocate_blocked_register num_stack_slots i =
    ...
    | _ ->
        ...
        allocate_stack_slot num_stack_slots i in
  ...
  let walk_interval num_stack_slots i =
    ...
    with
      Not_found ->
        ...
        allocate_blocked_register num_stack_slots i in
