...
let plt_stub_registers = [| r10; r11 |]
let plt_stub_registers_set = Reg.set_of_array plt_stub_registers
...
let calling_conventions first_int last_int first_float last_float
  ...
  for i = 0 to Array.length arg - 1 do
    ...
    | Val | Int | Addr as ty ->
        ...
        assert (not (Arch.use_plt ()
                     && Reg.Set.mem loc.(i) plt_stub_registers_set))
    | Float ->
...
let destroyed_at_oper = function
  ...
  | Iop(Iextcall {alloc; stack_ofs; }) ->
      ...
      else if Arch.win64 () then destroyed_at_c_call_win64
      else destroyed_at_c_call_sysv
  | Iop(Iintop(Idiv | Imod)) | Iop(Iintop_imm((Idiv | Imod), _))
        -> [| rax; rdx |]
  | Iop(Istore(Single, _, _)) -> [| rxmm15 |]
  | Iop(Ialloc _ | Ipoll _) ->
      if Arch.use_plt () then
        plt_stub_registers
      else
        [| r11 |]
...
let max_register_pressure =
  ...
  | Ialloc _ | Ipoll _ ->
      consumes ~int:(1 + (if Arch.use_plt () then 2 else 0)) ~float:0
