...
module Make (Arch : Operations.S) (Proc : module type of Processor) = struct
  ...
  (* Add edges from all instructions that define a pseudoregister [arg] being
     used as argument to node [node] (RAW dependencies *)
  ...
  method schedule_fundecl f =
    ...
    and schedule_block t ready_queue i try_nesting =
      if self#instr_in_basic_block i try_nesting then
        schedule_block t
          (self#add_instruction t ready_queue i) i.next try_nesting
      else begin
        let critical_outputs =
          ...
          | _ -> i.arg in
        List.iter
          (fun x -> ignore (longest_path critical_outputs x)) ready_queue;
