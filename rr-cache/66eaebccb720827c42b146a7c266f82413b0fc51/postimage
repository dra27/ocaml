...
CAMLexport void caml_main(char_os **argv)
{
  ...
  unsetenv("__OCAML_EXEC_FD=");
    ...
    if (fd < 0) {
      if (proc_self_exe != NULL) {
        ...
        /* Can't free exe_name yet because it will be used as argv0. It gets
           freed after the call to caml_locate_standard_library. */
        tofree = exe_name;
      }
    }
  ...
  /* Load the embedded runtime parameters. This is done _after_ the runtime's
     ...
     what should be being used to compute %standard_library. */
