...
(* Extract the name of a DLL from its mangled or external name. If the flag is
   [true] then the name is just the undecorated basename of the DLL (no -l and
   no .so). If the flag is [false] then the external name may include the DLL
