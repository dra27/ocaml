...
let link_compunit needs_stdlib output_fun currpos_fun
  ...
  List.fold_left
    (fun acc name ->
       ...
       acc || (name = "%standard_library_default"))
    needs_stdlib
...
let link_object output_fun currpos_fun file_name compunit =
  ...
  try
    let needs_stdlib =
      link_compunit false output_fun currpos_fun inchan file_name compunit
    in
    close_in inchan;
    needs_stdlib
  with
    ...
    let needs_stdlib =
      List.fold_left
        (fun needs_stdlib cu ->
           ...
           try
             link_compunit needs_stdlib output_fun currpos_fun inchan name cu
...
let link_files output_fun currpos_fun tolink =
  List.fold_left
    (fun acc thing ->
      link_file output_fun currpos_fun thing || acc) false tolink
...
let write_header outchan =
  ...
  let additional_runtimes =
    if search = Absolute then
      ...
      let runtime_id =
        let open Config in
        (* XXX There should be 3 runtimes in this list! *)
        Misc.RuntimeID.make_zinc
          ~static:(not static) ~int31:(Sys.int_size < 63) release_number
          ~is_release
      in
      [Printf.sprintf "ocamlrun%s-%s" !Clflags.runtime_variant runtime_id]
  in
...
let link_bytecode ?final_name tolink exec_name standalone =
  ...
  Misc.try_finally
    ...
    (fun () ->
       ...
       let needs_stdlib = link_files output_fun currpos_fun tolink in
