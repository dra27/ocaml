...
let mk_launch_method f =
  ...
  "<method>  Use <method> for the bytecode launcher rather than the mechanism \
...
third-party libraries such as Lwt, but with a different API."
    ...
    | "sh" ->
        launch_method := Some (Shebang None)
    | s when s <> "" && s.[0] = '/' ->
        launch_method := Some (Shebang (Some s))
    | _ ->
        Compenv.fatal
    ...
    | "enable" ->
        search_method := Absolute_then_search
    | "always" ->
